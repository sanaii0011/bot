...
نسخه تعمیرشده و سالم — وقت ایران، پومودورو واقعی، بدون خطا
"""
import asyncio
import csv
import io
...
def now_local() -> datetime:
    """زمان محلی تهران (برای ذخیره یکپارچه در دیتابیس)"""
...
async def db_get_notes(user_id: int) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM notes WHERE user_id=? ORDER BY id DESC LIMIT 20",
            (user_id,)
        ) as cur:
...
async def db_get_tasks(user_id: int) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def db_get_done_tasks(user_id: int, limit: int = 20) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def db_get_due_tasks() -> list:
...
async def db_get_subtasks(task_id: int) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
...
async def db_get_habits(user_id: int) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
...
async def db_get_users_list(page: int = 1, per_page: int = 5):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT user_id, first_name, username, level, xp FROM users LIMIT ? OFFSET ?",
            (per_page, offset)
        ) as cur:
...
async def db_get_user_full_details(target_user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            "SELECT title, status, category FROM tasks WHERE user_id=? ORDER BY id DESC LIMIT 10",
            (target_user_id,)
        ) as cur:
...
async def db_export_csv(user_id: int) -> str:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        writer.writerow(["ID", "Title", "Description", "Category", "Priority", "Urgent",
                         "Important", "DueDate", "Status", "Pomodoros", "CreatedAt", "DoneAt"])
        for r in rows:
            writer.writerow([r["id"], r["title"], r["description"], r["category"], r["priority"],
                             r["is_urgent"], r["is_important"], r["due_date"], r["status"],
                             r["pomodoros"], r["created_at"], r["done_at"]])
...
def fmt_task_advanced(t: dict, subtasks: list = None) -> str:
    if subtasks is None:
        subtasks = []
...
def task_action_kb(task_id: int, subtasks: list = None) -> InlineKeyboardMarkup:
    if subtasks is None:
        subtasks = []
    buttons = [
        [
    ...
    for t in tasks:
        ...
        try:
            ...
            await _bot.send_message(
                chat_id=t["user_id"],
                text="🔔 <b>زمان انجام این کار فرا رسید!</b>\n\n" + fmt_task_advanced(t, subs),
                parse_mode="HTML",
...
# ═══════════════════════════════════════════════════════════════════════════════
# ADD TASK WITH FULL BACK NAVIGATION
# ═══════════════════════════════════════════════════════════════════════════════
async def _send_step(update: Update, text: str, kb):
...
async def ask_title(update: Update, ctx):
    await _send_step(update,
                     "📝 <b>افزودن کار جدید (مرحله ۱ از ۶)</b>\n───────────────────────\nلطفاً <b>عنوان کار</b> را وارد کنید:",
                     step_back_kb(None))
    return ADD_TITLE


async def ask_desc(update: Update, ctx):
    text = (
        f"✅ <b>عنوان کار:</b> {ctx.user_data.get('title', '')}\n\n"
        "💬 <b>توضیحات تکمیلی (مرحله ۲ از ۶)</b> را وارد کنید:\n"
        "<i>(یا دستور /skip را ارسال کنید)</i>"
    )
    await _send_step(update, text, step_back_kb("title"))
    return ADD_DESC


async def ask_cat(update: Update, ctx):
    ...
    await _send_step(update, "🏷 <b>دسته‌بندی کار (مرحله ۳ از ۶)</b> را انتخاب کنید:", kb)
    return ADD_CAT


async def ask_pri(update: Update, ctx):
    kb = InlineKeyboardMarkup([
    ...
    await _send_step(update, "🎯 <b>اولویت کار (مرحله ۴ از )</b> را مشخص کنید:", kb)
    return ADD_PRI


async def ask_eisen(update: Update, ctx):
    kb = InlineKeyboardMarkup([
    ...
    await _send_step(update, "📐 <b>دسته‌بندی در ماتریس آیزنهاور (مرحله ۵ از ۶):</b>", kb)
    return ADD_EISENHOWER


async def ask_due(update: Update, ctx):
    msg = (
    ...
    await _send_step(update, msg, step_back_kb("eisen"))
    return ADD_DUE


async def ask_rec(update: Update, ctx):
    kb = InlineKeyboardMarkup([
    ...
    await _send_step(update, "🔁 آیا این کار نیاز به <b>تکرار خودکار</b> دارد؟", kb)
    return ADD_REC
...
async def add_got_title(update: Update, ctx):
    if update.message:
        ctx.user_data["title"] = update.message.text.strip()
    return await ask_desc(update, ctx)


async def add_got_desc(update: Update, ctx):
    if update.message:
...
async def add_got_cat(update: Update, ctx):
...
async def add_got_pri(update: Update, ctx):
    q = update.callback_query
    await q.answer()
...
async def add_got_eisenhower(update: Update, ctx):
    q = update.callback_query
    await q.answer()
...
async def add_got_due(update: Update, ctx):
    if update.message:
        if not update.message.text.startswith("/skip"):
            ...
            try:
                ...
                await update.message.reply_text(
                    "❌ فرمت ساعت نامعتبر است! مثال: <code>18:30</code>",
                    parse_mode="HTML", reply_markup=step_back_kb("eisen")
                )
                return ADD_DUE
        else:
...
async def add_got_rec(update: Update, ctx):
    q = update.callback_query
    await q.answer()
    rec = q.data.split(":")[1]
...
# ═══════════════════════════════════════════════════════════════════════════════
# GENERAL COMMANDS, LISTS, PROFILE & CSV
# ═══════════════════════════════════════════════════════════════════════════════
async def cmd_start(update: Update, _: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text, parse_mode="HTML", reply_markup=main_reply_keyboard(user.id))
...
async def cmd_list(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not tasks:
        no_task_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🌐 باز کردن وب‌اپ تودولیست", web_app=WebAppInfo(url=f"{WEBAPP_URL}?user_id={uid}"))]
        ])
    ...
    webapp_inline_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🌐 مدیریت گرافیکی کارهام در وب‌اپ", web_app=WebAppInfo(url=f"{WEBAPP_URL}?user_id={uid}"))]
    ])
...
# ═══════════════════════════════════════════════════════════════════════════════
async def quick_add_start(update: Update, ctx):
    intro = (
...
async def quick_add_process(update: Update, ctx):
...
async def _pomo_finish(user_id: int, task_id: int):
    try:
        await asyncio.sleep(POMO_SECONDS)
        await db_increment_pomo(task_id)
        await db_add_xp(user_id, 25)
        if _bot:
            await _bot.send_message(
                chat_id=user_id,
...
async def cb_pomo_start(update: Update, _: ContextTypes.DEFAULT_TYPE):
    ...
    minutes = POMO_SECONDS // 60
    await q.edit_message_text(
        q.message.text + f"\n\n🍅 <b>جلسه تمرکز {minutes} دقیقه‌ای شروع شد. پس از پایان، خودم خبرت می‌کنم!</b>",
        parse_mode="HTML"
    )
...
async def cb_add_habit_start(update: Update, ctx):
    q = update.callback_query
    await q.answer()
...
async def add_habit_process(update: Update, ctx):
...
async def cmd_eisenhower(update: Update, _: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    tasks = await db_get_tasks(uid)
    intro = "📐 <b>ماتریس ۴گانه آیزنهاور (Eisenhower Matrix)</b>\n───────────────────────\n"
...
async def cb_add_note_start(update: Update, ctx):
    q = update.callback_query
    await q.answer()
...
async def add_note_process(update: Update, ctx):
...
async def cb_add_subtask_start(update: Update, ctx):
    q = update.callback_query
    await q.answer()
    tid = int(q.data.split(":")[1])
...
async def add_subtask_process(update: Update, ctx):
    title = update.message.text.strip()
...
# ═══════════════════════════════════════════════════════════════════════════════
# WEB API SERVER (REST API + CORS + WEBAPP)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def handle_post_note(request):
    try:
        data = await request.json()
        await db_add_note(user_id=int(data["user_id"]), content=data["content"])
        return web.json_response({"status": "success"})
    except Exception as e:
        return web.json_response({"error": str(e)}, status=500)
    ...
    add_conv = ConversationHandler(
        ...
        states={
            ...
            ADD_DESC: [
                ...
                CallbackQueryHandler(ask_title, pattern=r"^goto:title$"),
                CallbackQueryHandler(cmd_cancel, pattern="^cancel_flow$")
            ],
            ADD_CAT: [
                CallbackQueryHandler(add_got_cat, pattern=r"^cat:"),
                CallbackQueryHandler(ask_desc, pattern=r"^goto:desc$"),
                CallbackQueryHandler(cmd_cancel, pattern="^cancel_flow$")
            ],
            ADD_PRI: [
                CallbackQueryHandler(add_got_pri, pattern=r"^pri:"),
                CallbackQueryHandler(ask_cat, pattern=r"^goto:cat$"),
                CallbackQueryHandler(cmd_cancel, pattern="^cancel_flow$")
            ],
            ADD_EISENHOWER: [
                CallbackQueryHandler(add_got_eisenhower, pattern=r"^eisen:"),
                CallbackQueryHandler(ask_pri, pattern=r"^goto:pri$"),
                CallbackQueryHandler(cmd_cancel, pattern="^cancel_flow$")
            ],
            ADD_DUE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, add_got_due),
                CommandHandler("skip", add_got_due),
                CallbackQueryHandler(ask_eisen, pattern=r"^goto:eisen$"),
                CallbackQueryHandler(cmd_cancel, pattern="^cancel_flow$")
            ],
            ADD_REC: [
                CallbackQueryHandler(add_got_rec, pattern=r"^rec:"),
                CallbackQueryHandler(ask_due, pattern=r"^goto:due$"),
                CallbackQueryHandler(cmd_cancel, pattern="^cancel_flow$")
            ],
        },
